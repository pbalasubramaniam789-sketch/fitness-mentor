...
import { WORKOUT_TYPES, WORKOUT_SUGGESTIONS, EXERCISE_DATABASE, EXERCISE_CATEGORIES } from '../utils/constants.js';
...
export function renderWorkout() {
  ...
  container.innerHTML = `
    ...
    `<option value="${key}">${data.icon} ${data.label}</option>`
  ).join('')}
              </select>
            </div>
            ...
            <div class="form-group" id="subcategory-group" style="display: none;">
              ...
              <select id="exercise-subcategory" class="form-select">
                <option value="">Select subcategory</option>
              </select>
            </div>

            <div class="form-group" id="exercise-group" style="display: none;">
              <label class="form-label required">Exercise</label>
              ...
              <tbody>
                ${workouts.map(workout => `
                  <tr>
                    ...
                    <td style="color: var(--text-secondary);">${workout.exerciseName || '-'}</td>
...
function setupWorkoutForm(profile) {
  ...
  const subcategorySelect = document.getElementById('exercise-subcategory');
  ...
  categorySelect.addEventListener('change', function () {
    ...
    const categoryData = EXERCISE_DATABASE[category];
    ...
    if (categoryData && typeof categoryData === 'object' && !Array.isArray(categoryData)) {
      ...
      Object.entries(categoryData).forEach(([key, value]) => {
        ...
        // Get label from EXERCISE_CATEGORIES
        const catInfo = EXERCISE_CATEGORIES[category];
        let label = key;

        if (catInfo?.subcategories) {
          if (catInfo.subcategories[key]?.label) {
            label = catInfo.subcategories[key].icon + ' ' + catInfo.subcategories[key].label;
          } else if (catInfo.subcategories[key]?.subcategories) {
            label = catInfo.subcategories[key].icon + ' ' + catInfo.subcategories[key].label;
          }
        }
      ...
      // No subcategories - directly show exercises
      exerciseGroup.style.display = 'block';
      categoryData.forEach(exercise => {
        const option = document.createElement('option');
  ...
  subcategorySelect.addEventListener('change', function () {
    ...
    const categoryData = EXERCISE_DATABASE[category];
    let exercises = categoryData[subcategory];

    // Handle nested subcategories (like arms -> biceps/triceps)
    if (exercises && typeof exercises === 'object' && !Array.isArray(exercises)) {
      exerciseSelect.innerHTML = '<option value="">Select specific exercise</option>';

      Object.entries(exercises).forEach(([key, exerciseList]) => {
        if (Array.isArray(exerciseList)) {
          const optgroup = document.createElement('optgroup');
          const catInfo = EXERCISE_CATEGORIES[category]?.subcategories?.[subcategory]?.subcategories?.[key];
          optgroup.label = catInfo ? `${catInfo.icon} ${catInfo.label}` : key;
...
function handleAddWorkout(e) {
  ...
  // Validate
  if (!type || !duration || !intensity || !category) {
  ...
  const workout = {
    type,
    category,
    exerciseName: exerciseName || 'Not specified',
    duration,
    intensity,
    caloriesBurned
  };
  ...
  document.getElementById('exercise-group').style.display = 'none';
